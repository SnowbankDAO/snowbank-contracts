...
contract CustomBond is Ownable {
    ...
    address immutable private snowbankDAO;
    address immutable private subsidyRouter; // pays subsidy in OHM to custom treasury
    address private snowbankTreasury; // receives fee
    ...
    /**
     *  @notice change address of Snowbank Treasury
     *  @param _snowbankTreasury address
     */
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        if(feeInPayout) {
            ...
            if(fee != 0) { // if fee, send to Snowbank treasury
                payoutToken.safeTransfer(snowbankTreasury, fee);
            }
        } else {
            customTreasury.sendPayoutTokens( payout );
            if(fee != 0) { // if fee, send to Snowbank treasury
                principalToken.safeTransferFrom( msg.sender, snowbankTreasury, fee );
            }
        }
    ...
    /**
     *  @notice calculate user's interest due for new bond, accounting for Snowbank Fee. 
    ...
    function payoutFor( uint _value ) public view returns ( uint _payout, uint _fee) {
        if(feeInPayout) {
            ...
            _fee = total.mul( currentSnowbankFee() ).div( 1e6 );
            _payout = total.sub(_fee);
        } else {
            _fee = _value.mul( currentSnowbankFee() ).div( 1e6 );
     ...
     *  @notice protocols using Snowbank Pro Max should be careful when quickly adding large %s to total supply
    ...
    /**
     *  @notice current fee Snowbank takes of each bond
     *  @return currentFee_ uint
     */
    function currentSnowbankFee() public view returns( uint currentFee_ ) {
...
contract SnowbankProMaxFactory {
    
    /* ======== STATE VARIABLS ======== */

    address immutable private snowbankProMaxFactoryStorage;
    address immutable private snowbankProMaxSubsidyRouter;
    address immutable public snowbankDAO;

    address public snowbankTreasury;

    /* ======== MODIFIERS ======== */

    modifier onlyDAO() {
        require( snowbankDAO == msg.sender, "caller is not the DAO" );
        _;
    }
    
    /* ======== CONSTRUCTION ======== */
    
    constructor(address _snowbankTreasury, address _snowbankProMaxFactoryStorage, address _snowbankProMaxSubsidyRouter, address _snowbankDAO) {
        require( _snowbankTreasury != address(0) );
        snowbankTreasury = _snowbankTreasury;
        require( _snowbankProMaxFactoryStorage != address(0) );
        snowbankProMaxFactoryStorage = _snowbankProMaxFactoryStorage;
        require( _snowbankProMaxSubsidyRouter != address(0) );
        snowbankProMaxSubsidyRouter = _snowbankProMaxSubsidyRouter;
        require( _snowbankDAO != address(0) );
        snowbankDAO = _snowbankDAO;
    }
    ...
    /**
        @notice chnage Snowbank Treasury address
        @param _snowbankTreasury address
     */
    function changeTreasuryAddress(address _snowbankTreasury) external onlyDAO() {
        snowbankTreasury = _snowbankTreasury;
    }
    ...
    function createBondAndTreasury(address _payoutToken, address _principalToken, address _initialOwner, uint[] calldata _tierCeilings, uint[] calldata _fees, bool _feeInPayout) external onlyDAO() returns(address _treasury, address _bond) {
        ...
        CustomBond bond = new CustomBond(address(treasury), _principalToken, snowbankTreasury, snowbankProMaxSubsidyRouter, _initialOwner, snowbankDAO, _tierCeilings, _fees, _feeInPayout);
        
        return ISnowbankProMaxFactoryStorage(snowbankProMaxFactoryStorage).pushBond(
    ...
    function createBond(address _principalToken, address _customTreasury, address _initialOwner, uint[] calldata _tierCeilings, uint[] calldata _fees, bool _feeInPayout ) external onlyDAO() returns(address _treasury, address _bond) {
        ...
        CustomBond bond = new CustomBond(_customTreasury, _principalToken, snowbankTreasury, snowbankProMaxSubsidyRouter, _initialOwner, snowbankDAO, _tierCeilings, _fees, _feeInPayout);
